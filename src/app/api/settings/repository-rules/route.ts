...
import { parseGitHubRepositoryInput } from '@/entities/github'
...
export async function POST(request: Request) {
  ...
  const repositoryInput =
    ...
    typeof body.repository === 'string'
      ...
      : null
  const rule =
    typeof body === 'object' && body && 'rule' in body ? body.rule : null

  if (!repositoryInput) {
    return NextResponse.json(
      { error: 'A repository URL or owner/repo is required.' },
      { status: 400 },
    )
  }
  ...
  if (rule !== 'allow' && rule !== 'block') {
    return NextResponse.json(
      { error: 'The rule must be either "allow" or "block".' },
      { status: 400 },
    )
  }
...
function signedOutResponse() {
  return NextResponse.json(
    { error: 'Sign in to manage repository rules.' },
    { status: 401 },
  )
}
