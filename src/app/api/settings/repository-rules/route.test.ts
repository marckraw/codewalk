...
describe("/api/settings/repository-rules", () => {
  ...
  it("requires authentication", async () => {
    ...
    expect(
      (await POST(jsonRequest({ repository: "https://github.com/acme/widgets", rule: "allow" }))).status,
    ).toBe(401);
    expect(upsertRepositoryReviewRule).not.toHaveBeenCalled();
  });

  it("creates a rule from a pasted repository URL", async () => {
    const response = await POST(jsonRequest({ repository: "https://github.com/Acme/Widgets", rule: "allow" }));
  ...
  it("creates a block rule from the owner/repo shorthand", async () => {
    const response = await POST(jsonRequest({ repository: "ef-global/noisy-repo", rule: "block" }));
  ...
  it("rejects invalid input", async () => {
    ...
    expect((await POST(jsonRequest({ repository: "not a repo", rule: "allow" }))).status).toBe(400);
    expect((await POST(jsonRequest({ repository: "acme/widgets", rule: "maybe" }))).status).toBe(400);
    expect(upsertRepositoryReviewRule).not.toHaveBeenCalled();
  });
});
