...
export async function POST(request: Request) {
  ...
  if (!config.ok) {
    ...
    return NextResponse.json(
      { error: config.message, missingKeys: config.missingKeys },
      { status: 503 },
    )
  }
  ...
  if (jsonText === null) {
    ...
    return NextResponse.json(
      { error: 'GitHub webhook payload was empty.' },
      { status: 400 },
    )
  }
  ...
  try {
    ...
    logCodewalkWarning('codewalk.github_webhook.invalid_json')
    return NextResponse.json(
      { error: 'GitHub webhook payload must be JSON.' },
      { status: 400 },
    )
  }
  ...
  if (!resolved.ok) {
    ...
    return NextResponse.json(
      { reason: resolved.reason, status: 'ignored' },
      { status: 202 },
    )
  }
  ...
  try {
    ...
    if (!access.allowed) {
      ...
      return NextResponse.json(
        { reason: access.reason, status: 'ignored' },
        { status: 202 },
      )
    }
