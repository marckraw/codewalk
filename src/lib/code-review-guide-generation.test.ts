...
import { AgentsDaemonClientError } from "@/lib/agents-daemon/client";
import {
  CodeReviewGuideGenerationError,
  buildRepositoryUrlFromSnapshot,
  generateAndPersistCodeReviewGuide,
} from "./code-review-guide-generation";

vi.mock("server-only", () => ({}));

vi.mock("@/lib/agents-daemon/config", () => ({
  getAgentsDaemonConfig: vi.fn(),
}));

vi.mock("@/lib/agents-daemon/client", async () => {
  const actual = await vi.importActual<typeof import("@/lib/agents-daemon/client")>("@/lib/agents-daemon/client");
...
vi.mock("@/lib/db/code-review-guides", () => ({
...
import { createAgentsDaemonClient } from "@/lib/agents-daemon/client";
import { getAgentsDaemonConfig } from "@/lib/agents-daemon/config";
import {
  finishCodeReviewGuideGeneration,
  startCodeReviewGuideGeneration,
} from "@/lib/db/code-review-guide-generations";
import { persistCodeReviewGuide } from "@/lib/db/code-review-guides";
