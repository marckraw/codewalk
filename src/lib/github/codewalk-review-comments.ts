...
import type { GitHubIssueComment, GitHubRestClient } from "@/lib/github/server/rest-client";
...
export function buildCodewalkReviewCommentBody(input: CodewalkReviewCommentInput) {
  switch (input.state) {
    case "preparing":
      return [
...
export async function upsertCodewalkReviewComment(input: {
  ...
  github: Pick<GitHubRestClient, "createIssueComment" | "listIssueComments" | "updateIssueComment">;
  pullRequest: GitHubPullRequestRef;
}): Promise<GitHubIssueComment> {
  if (input.existingCommentId) {
    try {
      return await input.github.updateIssueComment(input.pullRequest, input.existingCommentId, input.body);
    } catch (error) {
  ...
  const existing = await findExistingCodewalkComment(input.github, input.pullRequest);

  if (existing) {
    return input.github.updateIssueComment(input.pullRequest, String(existing.id), input.body);
  }
...
async function findExistingCodewalkComment(
  ...
  return comments.find((comment) => comment.body.includes(CODEWALK_REVIEW_COMMENT_MARKER)) ?? null;
}
