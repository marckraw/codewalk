...
describe("planPierreDiffPerformance", () => {
  ...
  it("virtualizes large diffs before enabling worker highlighting", () => {
    const diff = buildLargePierreDiffFixture(PIERRE_DIFF_VIRTUALIZATION_LINE_THRESHOLD);
  ...
  it("uses worker-pool highlighting for very large diffs", () => {
    const diff = buildLargePierreDiffFixture(PIERRE_DIFF_WORKER_POOL_LINE_THRESHOLD);
