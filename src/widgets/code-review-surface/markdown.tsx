...
export function MarkdownText({ className, content }: MarkdownTextProps) {
  ...
  return (
    <div
      className={cn(
        'grid gap-2 [&>*:first-child]:mt-0 [&>*:last-child]:mb-0',
        className,
      )}
    >
      <ReactMarkdown
...
const MARKDOWN_COMPONENTS: Components = {
  ...
  ol: ({ children }) => (
    <ol className="my-1 grid list-decimal gap-1 pl-5">{children}</ol>
  ),
  p: ({ children }) => (
    <p className="my-1 leading-[inherit] break-words">{children}</p>
  ),
  ...
  ul: ({ children }) => (
    <ul className="my-1 grid list-disc gap-1 pl-5">{children}</ul>
  ),
}
