...
export function reviewWorkspaceRepoKey(
  item: Pick<ReviewWorkspaceSummary, 'owner' | 'repo'>,
): string {
  return `${item.owner}/${item.repo}`
}

/** Distinct `owner/repo` keys present in the list, alphabetically sorted. */
export function listReviewWorkspaceRepos(
...
export function filterReviewWorkspaceSummaries(
  ...
  return items.filter((item) => {
    ...
    if (
      filters.repo !== 'all' &&
      reviewWorkspaceRepoKey(item) !== filters.repo
    ) {
      return false
    }
...
export const REVIEW_RECENCY_GROUP_LABELS = [
  'Today',
  'Yesterday',
  'This week',
  'This month',
...
export type ReviewRecencyGroupLabel =
  (typeof REVIEW_RECENCY_GROUP_LABELS)[number]
 ...
 */
export function reviewRecencyGroupLabel(
  value: Date | string,
  now: Date,
 ...
 * first).
 */
export function groupReviewWorkspacesByRecency(
  items: ReviewWorkspaceSummary[],
  now: Date,
): ReviewRecencyGroup[] {
