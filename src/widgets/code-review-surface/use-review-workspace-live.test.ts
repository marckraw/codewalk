...
describe("useReviewWorkspaceLive", () => {
  ...
  it("polls a preparing workspace until it becomes ready, then stops", async () => {
    ...
    const { result } = renderHook(() => useReviewWorkspaceLive(makeWorkspace("preparing"), { autoGenerate: false }));
  ...
  it("does not poll an idle imported workspace", async () => {
    ...
    renderHook(() => useReviewWorkspaceLive(makeWorkspace("imported"), { autoGenerate: false }));
  ...
  it("optimistically shows preparing and starts polling when generation is marked started", async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(makeWorkspace("ready")));
    vi.stubGlobal("fetch", fetchMock);

    const { result } = renderHook(() => useReviewWorkspaceLive(makeWorkspace("imported"), { autoGenerate: false }));
