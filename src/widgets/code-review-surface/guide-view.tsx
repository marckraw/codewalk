...
export function GuideView({ getFileDiff, guide, isFileLoading, onSelectFile, renderFileRef, renderSectionRef }: GuideViewProps) {
              ...
              <div className="flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[11px] font-medium text-muted-foreground">
                    {String(index + 1).padStart(2, "0")} / {String(guide.sections.length).padStart(2, "0")}
                  </p>
                  <h2 className="mt-3 text-2xl leading-8 font-semibold">{section.title}</h2>
                  <MarkdownText className="mt-3 text-sm leading-6 text-muted-foreground" content={section.summary} />
                </div>
                <RiskBadge section={section} />
              </div>
              <MarkdownText className="mt-5 text-sm leading-6 text-foreground" content={section.narrative} />
  ...
  return (
    <div className="mt-3 grid gap-1">
      {section.checklist.map((item) => (
        <div key={item} className="flex min-w-0 items-start gap-2 text-[11px] leading-5 text-muted-foreground">
