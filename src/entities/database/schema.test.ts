...
describe('database schema', () => {
  it('uses stable table names for core persisted objects', () => {
    ...
    expect(getTableName(codeReviewGuideGenerations)).toBe(
  ...
  it('tracks guide generation status separately from daemon guide data', () => {
    ...
    expect(codeReviewGuideGenerations.requestedByUserId.name).toBe(
    ...
    expect(codeReviewGuideGenerations.githubCommentId.name).toBe(
      'github_comment_id',
    )
    expect(codeReviewGuideGenerations.githubCommentUrl.name).toBe(
      'github_comment_url',
    )
  })
})
