...
export class GitHubRestClient {
  ...
  async getRepository(
    ref: Pick<GitHubPullRequestRef, 'owner' | 'repo'>,
  ...
  async listPullRequestFiles(
    ...
    const files = await this.requestPaginated<GitHubPullRequestFileResponse>(
  ...
  async listPullRequestCommits(
    ref: GitHubPullRequestRef,
  ): Promise<NormalizedPullRequestCommit[]> {
    const commits =
      await this.requestPaginated<GitHubPullRequestCommitResponse>(
        `${pullRequestPath(ref)}/commits`,
      )
  ...
  async listPullRequestComments(
    ref: GitHubPullRequestRef,
  ): Promise<NormalizedPullRequestComment[]> {
    const [issueComments, reviewComments] = await Promise.all([
      this.requestPaginated<GitHubIssueCommentResponse>(issueCommentsPath(ref)),
      this.requestPaginated<GitHubReviewCommentResponse>(
        `${pullRequestPath(ref)}/comments`,
      ),
    ])
  ...
  async listIssueComments(
    ...
    const comments = await this.requestPaginated<GitHubIssueCommentResponse>(
      issueCommentsPath(ref),
    )
  ...
  async updateIssueComment(
    ...
    const comment = await this.request<GitHubIssueCommentResponse>(
      issueCommentPath(ref, commentId),
      {
        body: { body },
        method: 'PATCH',
      },
  ...
  async getPullRequestSnapshot(
    ref: GitHubPullRequestRef,
  ): Promise<NormalizedPullRequestSnapshot> {
  ...
  private async requestPaginated<T>(path: string): Promise<T[]> {
    ...
    while (nextPath) {
      ...
      if (!Array.isArray(body)) {
        throw new Error(
          'GitHub returned an unexpected non-array paginated response.',
        )
      }
