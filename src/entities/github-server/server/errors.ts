...
export function missingGitHubAuthError() {
  return new GitHubClientError(
    'missing_auth',
    'Configure GITHUB_BOT_TOKEN before importing a pull request.',
  )
}
...
export function mapGitHubErrorResponse(response: Response, body: unknown) {
  ...
  const documentationUrl =
    typeof parsedBody?.documentation_url === 'string'
      ? parsedBody.documentation_url
      : undefined
  ...
  if (response.status === 401) {
    return new GitHubClientError(
      'missing_auth',
      'GitHub rejected the Codewalk bot token.',
      {
        body,
        documentationUrl,
  ...
  if (
    ...
    return new GitHubClientError(
      'rate_limited',
      'GitHub rate limit reached. Try again after the reset window.',
      {
        body,
        documentationUrl,
        retryAfterSeconds,
  ...
  if (response.status === 403) {
    return new GitHubClientError(
      'missing_scope',
      message ??
        'GitHub denied access. The Codewalk bot token may need repository read scope or org SSO approval.',
