...
describe('GitHubRestClient', () => {
  ...
  it('checks repository access from the server-side REST endpoint', async () => {
    ...
    await expect(
      client.getRepository({ owner: 'openai', repo: 'codex' }),
    ).resolves.toEqual({
  ...
  it('fetches changed files, patches, commits, and both comment feeds', async () => {
    const { client, requests } = createClient([
      ...
      jsonResponse([
        {
          ...
          commit: {
            author: { date: '2026-01-03T10:00:00Z' },
      ...
      jsonResponse([
        { body: 'Ship it', created_at: '2026-01-04T10:00:00Z', id: 1 },
      ]),
      jsonResponse([
        {
          body: 'Nit',
  ...
  it.each([
    ...
    'maps GitHub %s responses to app errors',
