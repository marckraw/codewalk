...
describe('Codewalk review comments', () => {
  it('builds stable PR comment bodies with a hidden marker', () => {
    ...
    expect(
      buildCodewalkReviewCommentBody({ reviewUrl, state: 'preparing' }),
    ).toContain(CODEWALK_REVIEW_COMMENT_MARKER)
    expect(
      buildCodewalkReviewCommentBody({ reviewUrl, state: 'preparing' }),
    ).toContain('is preparing')
    expect(
      buildCodewalkReviewCommentBody({ reviewUrl, state: 'ready' }),
    ).toContain('is ready')
    expect(
      buildCodewalkReviewCommentBody({
        error: 'Daemon failed',
  ...
  it('builds review deep links from the app base URL', () => {
    expect(
      buildCodewalkReviewUrl({
        appBaseUrl: 'https://codewalk.example.com/',
        snapshotId: 'snapshot-id',
      }),
    ).toBe('https://codewalk.example.com/review/snapshot-id')
    expect(
      getCodewalkAppBaseUrl({
        NEXT_PUBLIC_APP_URL: 'https://codewalk.example.com/path?debug=1',
      }),
    ).toBe('https://codewalk.example.com/path')
  })
  ...
  it('updates a stored comment id when available', async () => {
    const github = {
      ...
      updateIssueComment: vi
        .fn()
        .mockResolvedValue({ body: 'Ready', htmlUrl: 'url', id: 10 }),
    }
  ...
  it('recovers by marker when the stored comment id is missing', async () => {
    const github = {
      ...
      updateIssueComment: vi
        .fn()
        .mockResolvedValue({ body: 'Ready', htmlUrl: 'url', id: 7 }),
    }
  ...
  it('creates a comment when no stored or marked comment exists', async () => {
    const github = {
      createIssueComment: vi
        .fn()
        .mockResolvedValue({ body: 'Preparing', htmlUrl: 'url', id: 11 }),
      listIssueComments: vi.fn().mockResolvedValue([]),
    ...
    const github = {
      ...
      updateIssueComment: vi
        ...
        .mockRejectedValueOnce(
          new GitHubClientError('not_found', 'Missing comment.'),
        )
