...
export function getAgentsDaemonConfig(
  env: Record<string, string | undefined> = process.env,
): AgentsDaemonConfigResult {
  const baseUrl = resolveAgentsDaemonBaseUrl(env.AGENTS_DAEMON_BASE_URL)

  if (!baseUrl.ok) {
    const state =
      baseUrl.reason === 'missing' ? 'missing-base-url' : 'invalid-base-url'
    ...
    return {
      ...
      missingKeys:
        state === 'missing-base-url' ? ['AGENTS_DAEMON_BASE_URL'] : [],
      ok: false,
      state,
    }
  }
  ...
  if (!isCodeReviewGuideProvider(defaultProvider)) {
    return {
      message:
        'DEFAULT_GUIDE_PROVIDER must be one of: claude, codex, cursor, gemini.',
      missingKeys: [],
      ok: false,
...
export function getAgentsDaemonRequestTimeoutMs(
  env: Record<string, string | undefined> = process.env,
) {
  return resolvePositiveIntegerEnv(
    env,
    'AGENTS_DAEMON_REQUEST_TIMEOUT_MS',
    DEFAULT_AGENTS_DAEMON_REQUEST_TIMEOUT_MS,
  )
}

function isCodeReviewGuideProvider(
  value: string,
): value is CodeReviewGuideProvider {
  return (
    value === 'claude' ||
    value === 'codex' ||
    value === 'cursor' ||
