...
export function resolveAgentsDaemonBaseUrl(value: string | null | undefined): AgentsDaemonBaseUrlResolution {
...
export function parseAgentsDaemonMeta(value: unknown): AgentsDaemonMeta {
  ...
  return {
    ...
    deployment: {
      ...
      sharedAcrossTeams: requireBoolean(deployment.sharedAcrossTeams, "deployment.sharedAcrossTeams"),
    },
    git: {
      githubAuthenticated: requireBoolean(git.githubAuthenticated, "git.githubAuthenticated"),
    },
    name: requireString(obj.name, "name"),
    providers: obj.providers,
    runtime: {
      activeSessions: requireNumber(runtime.activeSessions, "runtime.activeSessions"),
      host: requireString(runtime.host, "runtime.host"),
      maxConcurrentAgents: requireNumber(runtime.maxConcurrentAgents, "runtime.maxConcurrentAgents"),
      port: requireNumber(runtime.port, "runtime.port"),
      uptimeSeconds: requireNumber(runtime.uptimeSeconds, "runtime.uptimeSeconds"),
    },
    version: requireString(obj.version, "version"),
  };
}

export function parseAgentsDaemonGenerateGuideResult(value: unknown): AgentsDaemonGenerateGuideResult {
...
function parseGuide(value: unknown): AgentsDaemonCodeReviewGuide {
  ...
  return {
    ...
    pullRequest: parsePullRequest(obj.pullRequest),
    pullRequestNumber: requireNumber(obj.pullRequestNumber, "guide.pullRequestNumber"),
...
function parsePullRequest(value: unknown): CodeReviewGuidePullRequestMetadata {
  ...
  if (state !== "open" && state !== "closed" && state !== "merged" && state !== "unknown") {
  ...
  return {
    ...
    headRepositoryName: optionalString(obj.headRepositoryName, "pullRequest.headRepositoryName"),
    headRepositoryOwner: optionalString(obj.headRepositoryOwner, "pullRequest.headRepositoryOwner"),
    number: requireNumber(obj.number, "pullRequest.number"),
    provider: requireLiteral(obj.provider, "github", "pullRequest.provider"),
    repositoryName: requireString(obj.repositoryName, "pullRequest.repositoryName"),
    repositoryOwner: requireString(obj.repositoryOwner, "pullRequest.repositoryOwner"),
    state,
...
function parseCacheIdentity(value: unknown): CodeReviewCacheIdentity {
  ...
  return {
    comparisonPoint: optionalString(obj.comparisonPoint, "cacheIdentity.comparisonPoint"),
    comparisonRef: optionalString(obj.comparisonRef, "cacheIdentity.comparisonRef"),
    workingTreeVersionToken: requireString(obj.workingTreeVersionToken, "cacheIdentity.workingTreeVersionToken"),
  };
}
...
function parseSections(value: unknown): AgentsDaemonCodeReviewGuideSection[] {
  ...
  return value.map((item, index) => {
    ...
    return {
      ...
      riskRationale: requireString(obj.riskRationale, `sections.${index}.riskRationale`),
...
function parseGuideFiles(value: unknown, field: string): AgentsDaemonCodeReviewGuideFile[] {
  if (!Array.isArray(value)) {
...
function parseFileEntries(value: unknown): CodeReviewGuideSummary["files"] {
  ...
  return value.map((item, index) => {
    ...
    const previousFile = optionalString(obj.previousFile, `summary.files.${index}.previousFile`);
...
function parseRiskLevel(value: unknown, field: string): CodeReviewGuideRiskLevel {
