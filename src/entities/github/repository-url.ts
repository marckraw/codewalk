...
export function parseGitHubRepositoryInput(
  ...
  if (!owner || !repo || rest.length > 0) {
    return {
      error:
        'Use a URL like https://github.com/owner/repo or the owner/repo shorthand.',
      ok: false,
    }
  }
...
function buildRepositoryRef(
  ...
  if (!githubNamePattern.test(owner) || !githubNamePattern.test(repo)) {
    return {
      error: 'The owner or repository name contains unsupported characters.',
      ok: false,
    }
  }
