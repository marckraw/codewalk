...
describe('OpenPullRequestDialog', () => {
  ...
  it('opens a focused PR URL dialog from the header command', async () => {
    ...
    expect(
      screen.getByRole('dialog', { name: 'Open pull request' }),
    ).toBeInTheDocument()
    expect(screen.getByLabelText('Pull request URL')).toHaveAttribute(
      'type',
      'url',
    )
  ...
  it('shows a user-facing error before submitting unsupported URLs', async () => {
    ...
    expect(
      await screen.findByText(
        'Only github.com pull request URLs are supported in the MVP.',
      ),
    ).toBeInTheDocument()
  ...
  it('submits valid GitHub PR URLs to the import route', async () => {
    ...
    await userEvent.type(
      screen.getByLabelText('Pull request URL'),
      'https://github.com/openai/codex/pull/24',
    )
    await userEvent.click(screen.getByRole('button', { name: 'Import PR' }))
    ...
    expect(
      await screen.findByText(
        'Imported openai/codex#24 with 4 files, 3 commits, and 2 comments.',
      ),
    ).toBeInTheDocument()
  ...
  it('shows the HTTP status when the import route returns a non-JSON response', async () => {
    ...
    expect(
      await screen.findByText(
        'The pull request import request failed with HTTP 404.',
      ),
    ).toBeInTheDocument()
