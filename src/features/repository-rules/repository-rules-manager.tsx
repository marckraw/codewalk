...
export function RepositoryRulesManager({
  allowedOwner,
  initialRules,
}: RepositoryRulesManagerProps) {
  ...
  async function submitRule(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body = (await response.json()) as {
        error?: string
        rule?: RepositoryRuleItem
      }

      if (!response.ok || !body.rule) {
        setError(
          body.error ?? `Saving the rule failed with HTTP ${response.status}.`,
        )
        return
      }
  ...
  async function removeRule(ruleId: string) {
    ...
    try {
      const response = await fetch(
        `/api/settings/repository-rules/${encodeURIComponent(ruleId)}`,
        {
          method: 'DELETE',
        },
      )
      ...
      if (!response.ok) {
        ...
        setError(
          body.error ??
            `Removing the rule failed with HTTP ${response.status}.`,
        )
        return
      }
    ...
    <div className="grid max-w-3xl gap-4">
      <form className="grid gap-2" onSubmit={submitRule}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            disabled={isSubmitting || !repositoryInput.trim()}
            type="submit"
            variant="primary"
          >
            ...
            >
              <div className="min-w-0">
                <p className="truncate font-mono text-sm">
                  {formatRepositoryRef(rule)}
                </p>
                <p className="text-xs text-[var(--muted)]">
                  {describeRule(rule, allowedOwner)}
                </p>
              </div>
...
function buildRuleHint(input: {
  ...
  const insideAllowedOwner =
    parsed.repository.owner.toLowerCase() === input.allowedOwner.toLowerCase()
...
function describeRule(rule: RepositoryRuleItem, allowedOwner: string | null) {
  if (!allowedOwner) {
    return rule.rule === 'allow'
      ? 'Whitelisted for guided reviews'
      : 'Excluded from guided reviews'
  }

  const insideAllowedOwner =
    rule.owner.toLowerCase() === allowedOwner.toLowerCase()
