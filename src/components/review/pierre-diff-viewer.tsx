...
function renderDiffHeader({ file, loading = false, title }: { file: string; loading?: boolean; title: string }) {
  return (
    <div className="shrink-0 border-b border-border px-3 py-2">
      <div className="flex min-w-0 items-center gap-2">
        <p className="min-w-0 flex-1 truncate font-mono text-[11px] text-foreground" title={file}>
          {file}
        </p>
        {loading ? <Loader2 className="size-3.5 shrink-0 animate-spin text-muted-foreground" /> : null}
      </div>
      <p className="mt-1 text-[10px] text-muted-foreground uppercase tracking-wider">{title}</p>
    </div>
  );
}

function renderPierreDiffPerformanceShell({ children, virtualize }: { children: ReactNode; virtualize: boolean }) {
  if (!virtualize) {
    return children;
  }
...
export function buildPierrePatch(input: { diff: string; file: string }): string | null {
  const diff = input.diff.trimEnd();
  if (!diff || !diff.includes("@@")) {
    return null;
  }

  if (diff.startsWith("diff --git ") || diff.startsWith("--- ") || diff.includes("\n--- ")) {
    return diff;
  }

  return [`diff --git a/${input.file} b/${input.file}`, `--- a/${input.file}`, `+++ b/${input.file}`, diff].join("\n");
}
