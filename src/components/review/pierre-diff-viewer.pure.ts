 ...
 */
export function describeMissingPierrePatch(input: { diff: string; fileStatus?: string | null }): string | null {
  ...
  if (/^Binary files /m.test(input.diff) || input.diff.includes("GIT binary patch")) {
