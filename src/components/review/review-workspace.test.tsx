...
vi.mock("./pierre-diff-viewer", () => ({
  PierreDiffViewer: ({ file }: { file: string | null }) => <div data-testid="diff-pane">{file ?? "no-file"}</div>,
}));

vi.mock("./changed-files-tree", () => ({
  ChangedFilesTree: ({ files, onSelectFile }: { files: { file: string }[]; onSelectFile?: (path: string) => void }) => (
    <div data-testid="file-tree">
      {files.map((entry) => (
        <button key={entry.file} onClick={() => onSelectFile?.(entry.file)} type="button">
          {entry.file}
        </button>
      ))}
    </div>
...
function makeWorkspace(): ReviewWorkspaceModel {
  return {
    files: [
      { id: "f1", path: "src/a.ts", status: "modified", patch: "@@ -1 +1 @@\n-a\n+b" },
      { id: "f2", path: "src/b.ts", status: "added", patch: "@@ -0,0 +1 @@\n+x" },
    ],
    generation: null,
    guide: {
      generatedBy: "agent",
      sections: [
        {
          checklist: ["check one"],
          files: [{ order: 0, path: "src/a.ts", reason: "why", status: "modified" }],
          id: "sec-1",
...
describe("ReviewWorkspace", () => {
  ...
  it("switches to the diff view and shows the selected file", async () => {
    ...
    await user.click(within(screen.getByTestId("file-tree")).getByRole("button", { name: "src/b.ts" }));
